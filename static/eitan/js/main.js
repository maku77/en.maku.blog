$(function() {
    ...
    var $judge = $('#judge').click(function(){window.location='./';});
    var $answer = $('#answer');
    var state = 'try';  // 'try' => 'hint' => 'answer'

    // Score class
    var Score = function() {
        this.STORAGE_KEY = 'eitanScore';
        this.storage = localStorage;
        var history = this.storage.getItem(this.STORAGE_KEY);
        this.history = history ? JSON.parse(history) : {};
    };

    Score.prototype = {
        // The number of words the user answered correctly.
        count: function() {
        ...
        // Set a word as answered correctly.
        correct: function(word) {
            this.history[word] = true;
            this.save();
        },
        // Set a word as answered incorrectly.
        incorrect: function(word) {
            delete this.history[word];
            this.save();
        },
        clear: function() {
            this.history = {};
    ...
    function showScore() {
        var $score = $('#score');
        $score.text(score.count() + ' / ' + wordList.length);
    }
    showScore();
    ...
    function updateProblem() {
        var w = getRandomWord();
        // remove legend in a problem sentense
        var prob = w[1];
        prob = prob.replace(/\bn\.\s*/g, '');
        prob = prob.replace(/\bv\.\s*/g, '');
        prob = prob.replace(/\bad\.\s*/g, '');
        prob = prob.replace(/\bprep\.\s*/g, '');
        prob = prob.replace(/\bpro\.\s*/g, '');
        prob = prob.replace(/\bconj\.\s*/g, '');
        // remove examples
        prob = prob.replace(/\s\(".+"\)/, '');
        $problem.text(prob);
        answer = w[0];
    ...
    updateProblem();
    $('#amazon embed').attr('bgcolor', 'black');
    ...
    $inputForm.submit(function(event) {
        ...
        if (input === answer) {
            $userInput.css('background', 'green');
            //$userInput.attr('disabled', 'disabled');
            $judge.text('正解！ 次の問題へ');
            $judge.css('color', 'lightgreen');
            state = 'answer';

            // TODO: show user's level
            score.correct(answer);
        } else {
            if (state !== 'hint') {
                $judge.text('ラストチャンス！');
                $judge.css('color', 'yellow');
                $userInput.val('');
                if (answer.length > 5) {
                    $userInput.attr('placeholder', answer[0] + answer[1]);
                } else {
                    $userInput.attr('placeholder', answer[0]);
                }
                $userInput.css('background', '#300');
                state = 'hint';
            } else {
                $judge.text('残念… 次の問題へ');
                $judge.css('color', 'red');
                $userInput.css('color', '#fcc');
                $userInput.css('background', 'darkred');
